  ...
  async (app) => {
    ...
    app.get('/catalogo/recategorizar/comparacion', async (_req, reply) => {
      const totalRow = await db.execute(
        sql`SELECT count(*)::int AS n FROM ${comerciosCatalogo}`,
      );
      const total = (totalRow.rows[0] as { n: number }).n;

      const recatRow = await db.execute(
        sql`SELECT count(*)::int AS n FROM ${comerciosCatalogo} WHERE recategorizado_at IS NOT NULL`,
      );
      ...
      // top diffs por par (categoria_actual_slug, categoria_nueva_slug)
